...
class SkillDetails(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        undone_tasks = tasks.filter(~Q(num_questions=F("num_answers")), level=skill_data[skill_id]["level"], skill__id=skill_id)
        if skill_id in user_skills_ids:
            quantity_tasks_undone = undone_tasks.count()
            if quantity_tasks_undone:
                progress = sum(undone_task.num_answers / undone_task.num_questions for undone_task in
                               undone_tasks) / quantity_tasks_undone
